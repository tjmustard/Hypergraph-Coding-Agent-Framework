...
| **`hyper-tutorial-run`** | `/hyper-tutorial-run` | Interactively walks the user through a tutorial generated by `/hyper-tutorial-generator`, one section at a time, with in-context Q&A and on-demand code samples. |
...
| **`hyper-tutorial-audit`** | `/hyper-tutorial-audit` | Run after `/hyper-tutorial-run` to turn tutorial failures into a structured, constraint-aware goal prompt. Interviews the user about their goal and allowed changes, then generates a paste-ready prompt for a new fix session. Never re-runs tutorial commands. |
| **`hyper-process-document`** | `/hyper-process-document` | Documents the process, methodology, and decisions of the current session as a reproducible narrative saved to `spec/process/`. Accepts an optional title argument. |
